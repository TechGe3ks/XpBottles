        ...
        if (item.getItemMeta().getLore() != null && item.getItemMeta().getLore().size() > 0) {
            ...
            for (String lore : item.getItemMeta().getLore()) {
                ...
                if (lore.toUpperCase().contains("EXP")) {
                    String[] expArray = lore.split(":")[1].trim().replace("§", "&").split("&");
                    String expAmount = "";

                    if (expArray.length > 1) {
                        expAmount = expArray[expArray.length - 1].substring(1, expArray[expArray.length - 1].length());
                    }

                    return Integer.parseInt(expAmount);
                }
            }
        }
